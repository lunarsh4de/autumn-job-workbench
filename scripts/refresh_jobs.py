...
def classify_job_type(job: dict[str, Any]) -> str:
    ...
    for job_type, keywords in JOB_TYPE_RULES:
        if any(keyword.lower() in haystack for keyword in keywords):
            return job_type
    tags = job.get("tags")
    tag_text = " ".join(clean(value, 200) for value in tags) if isinstance(tags, list) else clean(tags, 2000)
    for marker, job_type in TAG_FALLBACK_TYPES.items():
        if marker.lower() in tag_text.lower():
            return job_type
    if "产品/设计" in tag_text:
        design_markers = ("设计", "原画", "动画", "美术", "场景", "角色", "视频", "编导", "动作", "ui", "ux", "视觉", "3d")
        return "设计体验" if any(marker in haystack for marker in design_markers) else "产品项目"
    return "其他校招"
...
def is_mainland_location(value: Any) -> bool:
    ...
    if not location:
        return False
    if NON_MAINLAND_LOCATION_PATTERN.search(location):
        mainland_candidates = NON_MAINLAND_LOCATION_PATTERN.sub(" ", location)
        return bool(MAINLAND_SPECIFIC_LOCATION_PATTERN.search(mainland_candidates))
